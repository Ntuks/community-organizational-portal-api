...
describe('A user', () => {
  /* --------------- TESTING THE 'signup' RESOLVER ------------------------------*/
  test('should signup with new credentials.', async () => {
  ...
  test('should not signup with existing society credentials.', async () => {
    const response = await axios.post(url, {});

    const { data } = response;
    const { errors } = data;
    const { message } = errors[0];
    expect(message).toStrictEqual('Society with these credentials already exists.');
  });

  test('should not signup if email is invalid.', async () => {
    const response = await axios.post(url, {});
  ...
  test('should not signup if any of the fields are empty.', async () => {
    const response = await axios.post(url, {});
